        ...
        <ScrollArea className="h-48 rounded border border-border p-3">
          <div className="space-y-3">
            {plant.tweets.map((tweet) => (
              <div
                key={tweet.id}
                className={`p-3 bg-muted/50 rounded-lg border border-border/50 space-y-1 transition-all ${
                  tweet.id === newestTweetId
                    ? "animate-pulse bg-gradient-to-r from-yellow-100 via-yellow-200 to-yellow-100 dark:from-yellow-900/50 dark:via-yellow-800/50 dark:to-yellow-900/50 shadow-lg shadow-yellow-500/50 scale-[1.02] border-yellow-400"
                    : ""
                }`}
              >
